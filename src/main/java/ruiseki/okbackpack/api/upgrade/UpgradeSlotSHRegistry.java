...
public class UpgradeSlotSHRegistry {

    public interface ServerHandler {

        void handle(UpgradeSlotSH slot, PacketBuffer buf) throws IOException;
    }
    ...
    public static void handleServer(UpgradeSlotSH slot, int id, PacketBuffer buf) throws IOException {
        ServerHandler handler = SERVER_HANDLERS.get(id);
        if (handler != null) handler.handle(slot, buf);
    }

    public static void handleClient(UpgradeSlotSH slot, int id, PacketBuffer buf) throws IOException {

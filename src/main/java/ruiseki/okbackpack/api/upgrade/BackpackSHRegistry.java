...
public class BackpackSHRegistry {

    public interface ServerHandler {

        void handle(BackpackSH slot, PacketBuffer buf) throws IOException;
    }
    ...
    public static void handleServer(BackpackSH slot, int id, PacketBuffer buf) throws IOException {
        ServerHandler handler = SERVER_HANDLERS.get(id);
        if (handler != null) handler.handle(slot, buf);
    }

    public static void handleClient(BackpackSH slot, int id, PacketBuffer buf) throws IOException {
